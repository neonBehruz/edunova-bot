...
public static class DbInitializer
{
    public static async Task InitializeAndSeedAsync(AppDbContext dbContext, ILogger logger)
    {
        ...
        // Ensure questions exist across subjects
        int historyQuestionCount = await dbContext.Questions.IgnoreQueryFilters().CountAsync(q => q.SubjectId == 2);
        if (historyQuestionCount == 0)
        {
            logger.LogInformation("Populating multi-subject question bank...");
